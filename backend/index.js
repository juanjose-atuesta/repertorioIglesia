...
const campanita = new Set();

function notificar(evento, datos = {}) {
  const mensaje = `event: ${evento}\ndata: ${JSON.stringify(datos)}\n\n`;
  campanita.forEach(res => {
    try { res.write(mensaje); } catch (e) { console.log(e); }
  });
}
...
// Sirve el archivo real, pero buscando el nombre SIN extensión
app.get('/api/images/file/:nombre', (req, res) => {
  const nombreSinExtension = req.params.nombre;

  // Busca en fileNames un archivo cuyo nombre (sin extensión) coincida exactamente
  const archivoEncontrado = fileNames.find(f => {
    const sinExt = f.replace(/\.[^/.]+$/, ''); // quita la extensión
    return sinExt === nombreSinExtension;
  });
  ...
  res.sendFile(path.join(IMAGES_DIR, archivoEncontrado));
});
...
// AQUI EMPIEZA LA PARTE PARA PODER LEER LA CANCION 

const CURRENTSONG_PATH = path.join(__dirname, 'data', 'cancionActual.json');

function ensureCurrentSongFile() {
  if (!fs.existsSync(CURRENTSONG_PATH)) {
    fs.mkdirSync(path.dirname(CURRENTSONG_PATH), { recursive: true });
    fs.writeFIleSync(CURRENTSONG_PATH, '""')
  }
}

function leerCurrentSong() {
  ensureCurrentSongFile();
  return JSON.parse(fs.readFileSync(CURRENTSONG_PATH, 'utf-8'));
}


function actualizarCurrentSong(cancion) {

  ensureCurrentSongFile();
  fs.writeFileSync(CURRENTSONG_PATH, JSON.stringify(cancion, null, 1));
}
app.use(express.json());
app.get("/api/currentSong", (req, res) => {
  const cancion = leerCurrentSong();
  res.json(cancion);
});

app.post("/api/setNewCurrentSong", (req, res) => {
  const newCancion = req.body.nombre;
  if (!newCancion) {
    return res.status(400).json({ error: "se esperaba una cancion" });

  }
  actualizarCurrentSong(newCancion);
  res.json({ ok: true, cancion: newCancion });
